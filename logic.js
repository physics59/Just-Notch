...
function updateMapPlayer() {
  ...
  mapPlayer.style.left = Math.round((player.xCoordinate / 15) - 2.5) + "px";
  mapPlayer.style.top = Math.round((player.yCoordinate / 15) - 2.5) + "px";
}
...
function update() {
  let vDecay = 0.99609375;
  if (keys && (keys[40] || keys[83])) { player.velocity *= 0.95; }
  if (keys && (keys[38] || keys[87])) { player.velocity += 0.05; vDecay = 1;}
