...
let canvasData = {
  ...
  xVelocity: 0, // camera velocity X
  yVelocity: 0, // camera velocity Y
  stiffness: 0.0078125, // how strongly it pulls toward the player, 1/128
  damping: 0.875,    // friction/damping, 7/8
  canvasPosition: function() {
    this.center[0] += this.xVelocity;
    this.center[1] += this.yVelocity;
  },
  springVelocity: function() {
    let xDistance = player.xCoordinate - this.center[0];
    let yDistance = player.yCoordinate - this.center[1];
    this.xVelocity += xDistance * this.stiffness;
    this.yVelocity += yDistance * this.stiffness;
    this.xVelocity *= this.damping;
    this.yVelocity *= this.damping;
  },
  get corners() {
    let xOffset = this.xSize / 2;
    let yOffset = this.ySize / 2;
    let cornerArray = [];
    for (let signs of [[1, 1], [1, -1], [-1, -1], [-1, 1],]) {
      let newCorner = 
      [(xOffset * signs[0]) + this.center[0], (yOffset * signs[1]) + this.center[1]];
      cornerArray.push(newCorner);
    }
    return cornerArray;
  }
};
...
function resizeCanvas() {
  ...
  canvasData.xSize = window.innerWidth;
  canvasData.ySize = window.innerHeight;
...
function recenter() {
  ...
  player.atAngle = 0;
  canvasData.center = [0, 0];
  canvasData.xVelocity = 0;
  canvasData.yVelocity = 0;
}
...
function confirmPermClose() {
  ...
  text.innerHTML = `Are you sure you don't want to see this tip again? <button type="button" class="cancel cancel-confirm" onclick="saveTipPreference()">Yes</button>
   <a class="permaDismiss" onclick="revertChanges()">No</a>`;
}
...
function render() {
  ctx.setTransform(1, 0, 0, 1, window.innerWidth / 2 - canvasData.center[0], window.innerHeight / 2 - canvasData.center[1]);
  ctx.clearRect(canvasData.center[0] - (canvas.width / 2), canvasData.center[1] - (canvas.height / 2), canvas.width, canvas.height);
  for (let cloud of clouds) {
...
function renderMinimap() {
  ...
  for (let obj of objects) {
    ...
    let cornerArray = canvasData.corners;
    minimapCtx.beginPath();
    minimapCtx.strokeStyle = "black";
    minimapCtx.lineWidth = 10;
    minimapCtx.moveTo(cornerArray[0][0], cornerArray[0][1]);
    for (let i = 1; i < cornerArray.length; i++) {
      minimapCtx.lineTo(cornerArray[i][0], cornerArray[i][1]);
    }
